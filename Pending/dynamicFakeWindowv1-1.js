javascript:
/* 
 * dynamicFakeWindow v1.1 — GUI fake planner for the Rally Point
 *
 ...
 * with seconds), units and the target order. Settings persist per-world in
 * localStorage. Each run picks a target whose arrival — sent NOW, at the speed
 * of the slowest unit in the fake template — lands inside the window, then
 * fills coords + units. Target order: random, random without repeats, or the
 * order of the list. A target counts as used when you click Attack; when every
 * eligible target has been used the cycle starts over. Editing the list resets it.
 *
...
(function () {
  ...
  var core = {
    ...
    MODES: ['random', 'norepeat', 'order'],
    normalizeMode: function (m) { return core.MODES.indexOf(m) >= 0 ? m : 'random'; },

    /* Eligible targets not yet used (used = {coord: 1}). */
    fresh: function (eligible, used) {
      used = used || {};
      return eligible.filter(function (e) { return !used[e.c]; });
    },

    /* Pick the next target. random/norepeat: uniformly from `pool` (norepeat's
     * pool is core.fresh()); order: first of `pool` (plan() keeps list order). */
    pick: function (pool, rnd, mode) {
      if (!pool.length) return null;
      if (core.normalizeMode(mode) === 'order') return pool[0];
      return pool[Math.floor((rnd === undefined ? Math.random() : rnd) * pool.length)];
    },

    /* Choose from eligible honoring mode + used set. Returns
     * { choice, wrapped } — wrapped = every eligible target was already used,
     * so the caller should clear the used set (new cycle). */
    choose: function (eligible, mode, used, rnd) {
      mode = core.normalizeMode(mode);
      if (mode === 'random') return { choice: core.pick(eligible, rnd, mode), wrapped: false };
      var pool = core.fresh(eligible, used);
      var wrapped = !pool.length && eligible.length > 0;
      if (wrapped) pool = eligible;
      return { choice: core.pick(pool, rnd, mode), wrapped: wrapped };
    },

    /* Count how many of the list's coords are marked used. */
    usedCount: function (all, used) {
      used = used || {};
      var n = 0;
      all.forEach(function (c) { if (used[c]) n++; });
      return n;
    },
    ...
    STRINGS: {
      es: {
        ...
        modeLabel: 'Orden de objetivos:',
        modeRandom: 'Aleatorio',
        modeNoRepeat: 'Aleatorio (sin repetir)',
        modeOrder: 'En orden de la lista',
        modeHint: 'Un objetivo cuenta como usado al pulsar Atacar. Al agotarse, vuelve a empezar.',
        usedCount: function (n, total) { return n + '/' + total + ' usados'; },
        resetBtn: 'Reiniciar',
        ...
        progress: function (n, total) { return ' · ' + n + '/' + total + ' usados.'; },
        cycleRestart: ' Todos usados: nuevo ciclo.'
      },
      en: {
        ...
        modeLabel: 'Target order:',
        modeRandom: 'Random',
        modeNoRepeat: 'Random (no repeats)',
        modeOrder: 'List order',
        modeHint: 'A target counts as used when you click Attack. Once all are used, the cycle restarts.',
        usedCount: function (n, total) { return n + '/' + total + ' used'; },
        resetBtn: 'Reset',
        ...
        progress: function (n, total) { return ' · ' + n + '/' + total + ' used.'; },
        cycleRestart: ' All used: new cycle.'
      }
    }
  };
  ...
  /* Targets already attacked this cycle ({coord: 1}), per world. A coord is
   * marked when the Attack button / form submit fires — NOT when the script
   * fills (the speeds redo-fill and re-runs must not consume targets). */
  var USED_KEY = 'fakePlanner:used:' + game_data.world;
  function loadUsed() {
    try { var u = JSON.parse(localStorage.getItem(USED_KEY)); return (u && typeof u === 'object') ? u : {}; } catch (e) { return {}; }
  }
  function saveUsed(u) { try { localStorage.setItem(USED_KEY, JSON.stringify(u)); } catch (e) { } }
  function markUsed(coord) {
    var u = loadUsed();
    if (u[coord]) return;
    u[coord] = 1;
    saveUsed(u);
  }
  /* Hook Attack once per page: whichever fires first (button click or form
   * submit) records the coord currently in the target field. */
  function hookAttack() {
    var form = document.getElementById('command-data-form');
    var btn = document.getElementById('target_attack');
    var anchor = form || btn;
    if (!anchor || anchor.getAttribute('data-fp-hooked')) return;
    anchor.setAttribute('data-fp-hooked', '1');
    function onSend() {
      var t = document.getElementsByName('input')[0];
      var c = t && core.parseCoords(t.value)[0];
      if (c) markUsed(c);
    }
    if (btn) btn.addEventListener('click', onSend, true);
    if (form) form.addEventListener('submit', onSend, true);
  }
  function cfgValid(c) {
  ...
  function openPanel(cfg) {
    ...
    [['random', T.modeRandom], ['norepeat', T.modeNoRepeat], ['order', T.modeOrder]].forEach(function (o) {
      var opt = document.createElement('option');
      opt.value = o[0];
      opt.textContent = o[1];
      modeSel.appendChild(opt);
    });
    modeSel.value = core.normalizeMode(cfg.mode);
    var modeRow = field(T.modeLabel, modeSel);
    p.appendChild(modeRow);
    var modeInfo = document.createElement('div');
    modeInfo.style.cssText = 'font-size:11px;color:#555;margin-bottom:4px;display:flex;align-items:center;gap:6px;';
    var usedTxt = document.createElement('span');
    var resetBtn = document.createElement('button');
    resetBtn.type = 'button';
    resetBtn.textContent = T.resetBtn;
    resetBtn.style.cssText = 'cursor:pointer;font-size:11px;';
    resetBtn.addEventListener('click', function () { saveUsed({}); refreshMode(); });
    modeInfo.appendChild(usedTxt);
    modeInfo.appendChild(resetBtn);
    modeInfo.title = T.modeHint;
    function refreshMode() {
      var tracking = modeSel.value !== 'random';
      modeInfo.style.display = tracking ? 'flex' : 'none';
      var all = core.parseCoords(ta.value);
      usedTxt.textContent = T.usedCount(core.usedCount(all, loadUsed()), all.length);
    }
    modeSel.addEventListener('change', refreshMode);
    ta.addEventListener('input', refreshMode);
    refreshMode();
    p.appendChild(modeInfo);
    ...
    save.addEventListener('click', function () {
      var c = { coords: ta.value, startDate: sd.value, startTime: st.value, endDate: ed.value, endTime: et.value, units: readUnits(), mode: modeSel.value };
      ...
      /* a different target list starts a fresh cycle */
      if (core.parseCoords(cfg.coords).join(',') !== core.parseCoords(c.coords).join(',')) saveUsed({});
      saveCfg(c);
  ...
  function runFill(cfg) {
    ...
    var mode = core.normalizeMode(cfg.mode);
    var picked = core.choose(res.eligible, mode, loadUsed());
    var choice = picked.choice;
    if (picked.wrapped) saveUsed({});
    if (!choice) {
    ...
    hookAttack();
    var msg2 = T.filled(choice.c, fmt(choice.t), res.eligible.length, res.all.length);
    if (mode !== 'random') {
      msg2 += T.progress(core.usedCount(res.all, loadUsed()), res.all.length);
      if (picked.wrapped) msg2 += T.cycleRestart;
    }
    setStatus(msg2, 'green');
    var btn = document.getElementById('target_attack');
    if (btn) btn.focus();
  }
