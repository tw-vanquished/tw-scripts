...
//
// 2.1: catapult target. When a command carries catapults the details JSON also
// has catapult_target {id: building key, name: localized building name} — for
// tribe mates' shared commands too, not only your own. Exported per command as
// catapult_target (key, e.g. "farm") + catapult_target_name (e.g. "Granja"),
// null when the command has no catapults; the CSV gains a last CatapultTarget
// column (header-driven importers are unaffected).
//
...
// With catapults on board (attacks, shared ones included) there is additionally
//   "catapult_target":{"id":"main","name":"Edificio Principal"}
// (the key is absent otherwise). Counts are strings; every world unit is present. A body without village_start
...
function ioParseCommandDetails(data, villagesById, players, tzOffsetMs) {
    ...
    var detail = { originPlayer: "", originVillage: "", originCoords: "", arrival: "", arrivalEpochMs: null, units: {}, hidden: false,
                   catapultTarget: "", catapultTargetName: "" };
    ...
    var ct = data.catapult_target;
    if (ct && typeof ct === "object") {
        detail.catapultTarget = ct.id != null ? String(ct.id) : "";
        detail.catapultTargetName = ct.name != null ? String(ct.name) : "";
    } else if (typeof ct === "string") {
        detail.catapultTarget = ct;
    }
    return detail;
}
...
function ioBuildCsv(targetsOut, units) {
    var csv = "Target,TargetVillage,TargetOwner,Type,Size,Command,OriginCoords,OriginVillage,OriginPlayer,Arrival,ArrivesIn," + units.join(",") + ",CatapultTarget\n";
        ...
        if (tg.status != "ok") {
            ...
            for (var k = 0; k < tg.cmds.length; k++) {
                ...
                csv += base + ioCsvField(cmd.type) + "," + (cmd.size || "") + "," + ioCsvField(cmd.label) + "," +
                    ...
                    ioCsvField(d.catapultTargetName || d.catapultTarget || "") + "\n";
            }
        }
    }
    return csv;
...
// (null when the details fetch failed); 2.1 adds catapult_target (building key)
// + catapult_target_name (the game's localized name), both null without catapults.
            ...
            return {
                ...
                commands: tg.cmds.map(function (cmd) {
                    ...
                    return {
                        ...
                        units: unitObj,
                        catapult_target: d.catapultTarget || null,
                        catapult_target_name: d.catapultTargetName || null
                    };
                })
            };
        })
